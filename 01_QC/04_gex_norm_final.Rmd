...
```{r add cell cycle genes, messsage=F}
...
```{r create new score for regression, include=T, eval=F}
...
### 4. SCTransform
Keep all proliferation genes to stratify proliferating and non proliferating B cells.
```{r regress SCTransform, include=T}
# regress out cc_diff score, use all genes (residual.features=NULL) to calc residuals
split_sct = lapply(split_cc, SCTransform, vst.flavor = "v2", residual.features=NULL)
```
...
#split_sct = qread("./01_QC/rds/04_split_sct_noCCreg.qs")
```
...
ggsave("01_QC/figs/04_sctUMAP_BvsT_plot_noCCreg.png", BvsT_sct, width = 36, height = 5)
```
