---
title: "QC of composite lymphoma 10X multiome data: ATAC"
author: "kou"
date: "220102"
...
## QC of ATAC of composite LN samples
...
However, by default there are a few cutoffs
* TSS enrichment = 4
* 1000 < # unique Frags < 100,000
...
archR_unflt = ArchRProject(
  ...
  outputDirectory = "01_QC/ArchRproj_unflt",
  copyArrows = F)

# save
archR_unflt = saveArchRProject(ArchRProj = archR_unflt, 
                               outputDirectory = "01_QC/ArchRproj_unflt", 
                               load = F)

archR_unflt = loadArchRProject("01_QC/ArchRproj_unflt")
```
...
ggsave("01_QC/figs/02_fragsize_unflt.png", height = 2, width = 7)
```
...
ggsave("01_QC/figs/02_tssEnrich_unflt.png", height = 2.5, width = 7)
```
...
ggsave("01_QC/figs/02_unqFrag_vs_TSSenrich_unflt.png", height = 2.5, width = 7)
```
...
ggsave("01_QC/figs/02_vln_TSSenrich_unflt.png", height = 4, width = 4)
```
...
ggsave("01_QC/figs/02_vln_unqFrag_unflt.png", height = 4, width = 4)
```


### Filtering Doublets
Need to set the filter ratio, which would give the *max* number number of cells to filter out. The higher the ratio, the greater the number of removed cells. 

Selected different ratios to see how many cells would be eliminated. 
```{r flt dblt, include=T}
fltRatios = seq(1,3, by=0.5)

# get list of proj w/ different filter ratios
flt_proj = map(fltRatios, ~ filterDoublets(archR_unflt, filterRatio = .x))
```

TO DO: Plot number of cells that are removed w/ each filter ratio.
```{r plot dblt removal, include=T}
# set names
names(flt_proj) = paste0("filterRatio_", fltRatios)

# TO DO: get number of remaining cells/sample
```
...
archR_flt = saveArchRProject(ArchRProj = flt_proj$filterRatio_1.5, 
                             outputDirectory = "01_QC/ArchRproj_flt", 
                             load = T)
...
Performs better than ArchR when two similar cells (homotypic) are doublets. Use UNFILTERED ArchR project to compare
```{r amulet, include=T}
...
frags_unflt = lapply(frags_unflt, dropSeqlevels, value = c("chrX","chrY","chrM"), pruning.mode = "coarse")
...
# run amulet, 
amulet_res = lapply(frags_unflt, amulet)
...
saveRDS(toDISCARD, "01_QC/rds/02_toDISCARD.rds")
```
...
plot_mlt = tibble(cell_ids = cells_unflt,
                  is.multiplet = case_when(cells_unflt %in% setdiff(mlt_ArchR,mlt_both) ~ "ArchR Only",
                                           cells_unflt %in% setdiff(mlt_AMULET,mlt_both) ~ "AMULET Only",
                                           cells_unflt %in% mlt_both ~ "Both")) %>%
  dplyr::mutate(is.multiplet = replace_na(is.multiplet,"Neither (Singlets)"),
...
ggsave("./01_QC/figs/02_multiplets_summary.png",width=5,height = 3)
```
...
# overwrite 
archR_flt = saveArchRProject(ArchRProj = archR_flt2, 
                             outputDirectory = "01_QC/ArchRproj_flt", 
                             load = T)
```
